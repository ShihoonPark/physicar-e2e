"""Gated PilotNet V3 DAgger 0.50 m/s closed-loop validation."""
...
def load_v3_config(path: Path) -> InferenceConfig:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("version") != "pilotnet_inference_v3_dagger":
        raise ValueError("unexpected V3 inference config")
    if payload.get("smoke_speeds_mps") != [0.5, 0.5, 0.5] or payload.get("maximum_smoke_runs") != 3:
        raise ValueError("V3 permits at most three conditional 0.50 m/s runs")
    if payload.get("camera_only_model_observation") is not True:
        raise ValueError("V3 observation must remain camera-only")
    return InferenceConfig(payload)


def run_conditional_v3(client, model, config: InferenceConfig, persist=None) -> list[dict]:
    results: list[dict] = []
...
def aggregate(results: list[dict]) -> dict | None:
    ...
    ctes = [item["mean_cte_m"] for item in results]
    saturations = [item["steering_saturation_fraction"] for item in results]
    return {"result": "3/3 PASS", "lap_time_mean_s": statistics.fmean(times), "lap_time_std_s": statistics.pstdev(times),
            "mean_cte_mean_m": statistics.fmean(ctes), "mean_cte_std_m": statistics.pstdev(ctes),
            "worst_max_cte_m": max(item["max_cte_m"] for item in results),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run and (args.result.exists() or marker.exists()):
        print("ERROR: refusing another V3 live experiment", file=__import__("sys").stderr); return 2
    report = {"version": "pilotnet_e2e_v3_dagger", "generated_utc": datetime.now(timezone.utc).isoformat(), "result": "FAIL", "runs": []}
    client = None
    code = 2
    try:
        ...
        if args.preflight_only:
            ...
            marker.write_text(json.dumps({"status": "V3_LIVE_STARTED_DO_NOT_RETRY", "started_utc": datetime.now(timezone.utc).isoformat()}, indent=2) + "\n")
            def persist(results):
                report["runs"] = results
                args.result.write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
            report["runs"] = run_conditional_v3(client, model, config, persist)
